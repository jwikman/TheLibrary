# GitHub Copilot Instructions for AL Vibe Agents

## Compilation Instructions

### Single Project Repository

To compile the main App:
```bash
pwsh ./.github/.tmp/compile-app.ps1
```

To compile the Test App:
```bash
pwsh ./.github/.tmp/compile-test-app.ps1
```

## Additional Tools

- For a full list of available MCP servers and their recommended usage, see `.github/instructions/mcp-servers.instructions.md`. When running as the GitHub Coding agent, always consult this file to select the appropriate MCP server for your task.

## Before Committing Code

- When you have completed your code changes, you must compile both the main App and the Test App to ensure there are no warnings or errors.
...
## Translation and Localization Instructions

When asked to work with translations or localizations of the app (e.g., XLF files, language files, or related translation/localization tasks), you MUST follow the comprehensive instructions in `.github/instructions/translate-xlf.instructions.md`. This file contains the complete workflow, tools, and requirements for translating Business Central AL XLF localization files.
...
- Run compile/build steps for affected projects (main App and Test App) and ensure zero errors/warnings when code was modified (unless user explicitly allows warnings—otherwise treat warnings as issues to fix or justify).
- Perform minimal smoke validation for new/changed AL objects where feasible (e.g., ensure procedures compile, events wired, no unused labels created).
...
### 7. Translation / Localization Considerations
- If changes introduce user-facing text, use label variables and, if translation pipeline is in scope for the task, update or flag XLF sync steps (do NOT perform translation unless explicitly requested).

### 8. Performance & Safety
- For data operations, confirm filtering early, limited field loading, and set-based logic opportunities were considered (state them explicitly if relevant to the task).
- Highlight any potential long-running loops or locking risks introduced.

### 9. Proactive Adjacent Improvements
- After satisfying all explicit requirements, add only low-risk, clearly beneficial improvements (e.g., tiny helper extraction, missing XML docs for new public procedures) and list them separately as "Adjacency Improvements" so they are distinguishable from core scope.
